...
Linear Regression was used as a classification model to predict hours studied vs. exam score for a student. In our final model, we used many more predictors to better predict exam score, resulting in much better performance than the model with only 1 predictor. With Linear Regression, we perform it on a training and validation set, then a test set to evaluate our performance. When comparing true to predicted values in our validation, we get the following metrics in our primary lab.
...
However, the results were still quite poor, with ROC cruves barely better than random chance. We got the following performance metrics on the training dataset with the optimized precision and 5 neighbors.
...
During KNN, we tuend te hyperparameter of the number of neighbors by training the model with all numbers of neighbors from 1 to about 89, and their performance was evaluated on the validaion set using precision as the metric. The choice of precision was because there were so many falsely classified private school students as public school students in all performance metrics, so we wanted to minimize that error. We also tried accuracy, recall, and entropy. The choice of metric to evaluate the model on was also a hyperparameter that was tuned a little less methodically, but with justification.

In the neural network, the choice of epochs was chosen using early stopping, but the patience was tuned by trying a few different values (20, 35, 100) and evaluating the results on the test data.