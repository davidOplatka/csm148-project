...
During training, we fine-tuned the number of neighbors by training the model with k=1 to k=89, and their performance was evaluated on the validaion set using precision as the metric. The choice of precision was because there were many more falsely classified private school students when optimizing all performance metrics, so we hoped to minimize that error. The other metrics we tried were accuracy, recall, and entropy. The choice of metric to evaluate the model on was also a hyperparameter that was tuned a little less methodically, but with justification.
...
For validating and evaluating our model, we first noticed overfiiting issues since we had a low training loss and high test MSE. Therefore, we implemented **early stopping**, which monitored validation loss with a patience theshold of 35 epochs (we also tested patience of 20 and 100 epochs but 35 to yield the best results).
